...
class ResumesDeclinedTest < ActionController::IntegrationTest
  fixtures :all
  ...
  def test_should_decline_the_resume_using_xhr
    resume          = @resumes[0]
    total_comments  = resume.comments.size
    forwards        = resume.forwards.find_all {
                        |fwd| fwd.status == "SCHEDULED"
                      }
    forwards.each do |fwd|
      @logged_employee.declined  :forward_id => fwd.id,
                                 :status => "DECLINED",
                                 "resume[comment]".to_sym => "DECLINED: Resume declined using Ajax"
    end
