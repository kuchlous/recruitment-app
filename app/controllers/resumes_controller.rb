class ResumesController < ApplicationController
  ...
  def export_interviews
    ...
    end_date = Date.today
    Spreadsheet.client_encoding = 'UTF-8'
    output                      = "#{RAILS_ROOT}/tmp/interviews_#{Date.today.day}_#{Date.today.month}.xls"
    book                        = Spreadsheet::Workbook.new
    results = Interview.find(:all, :conditions => [ "interview_date >= '#{start_date}' AND interview_date <= '#{end_date}'"])
    results.uniq_by { |r| r.req_match.requirement_id }
  ...
  def export_as_xls_requirement
    ...
    req_forwards                = requirement.forwards
    req_forwards                += requirement.req_matches
    output                      = "#{RAILS_ROOT}/tmp/requirement_full_status_#{requirement.name.gsub(/[& -\/\\]/, '_')}.xls"
    forwarded_sheet             = book.create_worksheet :name => "Forwarded"
    shortlisted_sheet           = book.create_worksheet :name => "Shortlisted"
  ...
  def get_hr_matches_cached(status)
    @row_id_prefix = get_row_id_prefix(status)
    resumes = Resume.find_all_by_overall_status(status)
    forwards = []
    resumes.each do |r|
  ...
  def get_hr_matches(status)
    @row_id_prefix = get_row_id_prefix(status)
    forwards       = Forward.find_all_by_status(status).find_all { |f|
      f.resume.resume_overall_status == status.capitalize
    }
    forwards      += ReqMatch.find_all_by_status(status).find_all { |r|
