class RequirementsController < ApplicationController
  ...
  before_filter :get_all_objects,                     :only => [ :new, :create, :edit, :update ]
  before_filter :check_for_HR_or_ADMIN_or_REQMANAGER_or_BD, :only => [ :all_reqs ]

  def index
    @requirements = Requirement.find(:all, :conditions => { :status => "OPEN" } , :order => "employee_id")
