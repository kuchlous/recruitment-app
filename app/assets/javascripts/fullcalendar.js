...
function _renderDaySegs(segs, rowCnt, view, minLeft, maxLeft, getRow, dayContentLeft, dayContentRight, segmentContainer, bindSegHandlers, modifiedEventId) {
    ...
    if (colorArray.get(event.interviewer_id))
    {
      div_color = colorArray.get(event.interviewer_id);
    }
    else
    {
      div_color = '#'+(0x1000000+(Math.random())*0xffffff).toString(16).substr(1,6);
      colorArray.set(event.interviewer_id, div_color);
    }
		...
		html +=
			"<div class='" + className + event.className.join(' ') + "' style='position:absolute;z-index:8;left:"+left+"px;background-color:"+div_color+";border-color:"+div_color+"'>" +
...
function Agenda(element, options, methods) {
	...
	function renderSlotSegs(segs, modifiedEventId) {
		...
		for (i=0; i<segCnt; i++) {
			...
			html +=
				"<div class='" + className + event.className.join(' ') + "' style='position:absolute;z-index:8;top:" + top + "px;left:" + left + "px;background-color:"+div_color+";border-color:"+div_color+"'>" +
					"<a" + (event.url ? " href='" + htmlEscape(event.url) + "'" : '') + ">" +
